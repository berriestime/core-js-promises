...
function getFirstResolvedPromiseResult(promises) {
  return new Promise((resolve, reject) => {
    const promisesAlwaysResolved = promises.map((p) =>
      p.catch(() => undefined)
    );

    Promise.all(promisesAlwaysResolved).then((results) => {
      const fulfilledResults = results.filter((result) => result !== undefined);

      if (fulfilledResults.length > 0) {
        resolve(fulfilledResults[0]);
      } else {
        reject();
      }
    });
